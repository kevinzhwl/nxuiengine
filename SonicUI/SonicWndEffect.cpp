...
void CSonicWndEffect::OnTimerShowingGently()
{
	...
	{
		ShowWindow(m_hWnd, m_Effect.fadeout.nShowType);
	}
...
void CSonicWndEffect::UpdateLayer()
{
	...
	else
	{
		...
		SetLayeredWindowAttributes(m_hWnd, m_nKeyColor, bConstantAlpha, dwFlag);
	}
}
...
BOOL CSonicWndEffect::OnWndMsg(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_WINDOWPOSCHANGING:
		{
			return OnPosChange((LPWINDOWPOS)lParam);			
		}
		break;
...
void CSonicWndEffect::DrawLayer()
{
	if(m_hWnd == NULL)
	{
		return;
...
BOOL CSonicWndEffect::OnPosChange(LPWINDOWPOS pPos)
{
	BOOL bRet = FALSE;
	...
	g_UI.CallOldProc(m_hWnd, WM_WINDOWPOSCHANGING, NULL, (LPARAM)pPos);
	if(!(flags & SWP_NOMOVE))
	{
		if(m_Dib.IsValid())
		{
			m_rtCache.MoveToXY(pPos->x, pPos->y);
		}
	}
	if(!(flags & SWP_NOSIZE))
	{
		GetWindowRect(m_hWnd, rt);
		if(rt.Width() != pPos->cx || rt.Height() != pPos->cy)
		{
			if(m_Effect.dwEffectMask & WND_EFFECT_FRAME)
			{
			...
			if(m_Dib.IsValid())
			{
				...
				m_rtCache.SetRect(m_rtCache.left, m_rtCache.top, m_rtCache.left + pPos->cx, m_rtCache.top + pPos->cy);
				UpdateDibLayer(m_rtCache.left, m_rtCache.top, m_rtCache.Width(), m_rtCache.Height(), 0, 0, TRUE);
			}
		}
	}
	return bRet;
...
void CSonicWndEffect::FinalEffect(HDC hdc)
{
	...
	{
		m_Effect.pImg->Load(hdc, g_UI.m_rtUpdate.left, g_UI.m_rtUpdate.top, g_UI.m_rtUpdate.Width(), g_UI.m_rtUpdate.Height(), TRUE);
		m_Effect.pImg->Gray();
		m_Effect.pImg->SetBitsToDevice(hdc, g_UI.m_rtUpdate.left, g_UI.m_rtUpdate.top);
	}
...
BOOL CSonicWndEffect::EnableTransform(BOOL b, int nAngle /* = 0 */, RECT * pDest /* = NULL */, int nFrame /* = 0 */)
{
	...
	if(b)
	{
		...
		SetWindowPos(m_hWnd, NULL, -m_Effect.transform.rtOrigin.Width(), -m_Effect.transform.rtOrigin.Height(), 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
...
void CSonicWndEffect::RestoreTransform()
{
	...
	SetWindowPos(m_hWnd, NULL, m_Effect.transform.rtOrigin.left, m_Effect.transform.rtOrigin.top, 
		m_Effect.transform.rtOrigin.Width(), m_Effect.transform.rtOrigin.Height(), SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOREDRAW);
	m_Effect.transform.Clear();
}
...
void CSonicWndEffect::UpdateDibLayer(int x, int y, int cx, int cy, int xSrc /* = 0 */, int ySrc /* = 0 */, BOOL bRedraw /* = TRUE */)
{
	if(m_Dib.IsValid() == FALSE)
	{
		return;
...
void CSonicWndEffect::UpdateDibLayer(BOOL bRedraw /* = TRUE */)
{
	if(m_Dib.IsValid() == FALSE)
	{
		return;
...
void CSonicWndEffect::Transform()
{
	...
	SetWindowPos(m_Effect.transform.hHelp, NULL, x, y, m_Effect.transform.pImgTransform->GetWidth(), m_Effect.transform.pImgTransform->GetHeight(), 
...
ISonicImage * CSonicWndEffect::Snapshot()
{
	...
	SendMessage(m_hWnd, WM_PRINT, (WPARAM)pImg->GetDC(), PRF_CLIENT | PRF_NONCLIENT | PRF_ERASEBKGND | PRF_CHILDREN);
	if(m_Dib.IsValid() == FALSE)
	{
		if(m_nKeyColor != -1)
		{
...
void CSonicWndEffect::OnTimerSlide()
{
	...
	pEffect->UpdateDibLayer(x, y, cx, cy, x1, y1);
...
{
	BOOL bRet = TRUE;
	if(g_UI.m_rtUpdate.Width() == 0)
	{
		GetClientRect(m_hWnd, g_UI.m_rtUpdate);
	}
	if(m_Effect.dwEffectMask & WND_EFFECT_BKCOLOR)
	{
		...
		else
		{
			...
			::ExtTextOut(hdc, 0, 0, ETO_OPAQUE, g_UI.m_rtUpdate, NULL, 0, NULL);
		}
		bRet = FALSE;
	}
	if(m_pBg)
	{
		BitBlt(hdc, g_UI.m_rtUpdate.left, g_UI.m_rtUpdate.top, g_UI.m_rtUpdate.Width(), g_UI.m_rtUpdate.Height(), m_pBg->GetDC(), 
			g_UI.m_rtUpdate.left, g_UI.m_rtUpdate.top, SRCCOPY);
		bRet = FALSE;
	}
	return bRet;
}
...
BOOL CSonicWndEffect::DirectTransform(int nAngle, RECT * pDest, int nFrame)
{
	...
	else
	{
		...
		SetWindowPos(m_Effect.transform.hHelp, NULL, m_Effect.transform.rtOrigin.left, m_Effect.transform.rtOrigin.top, 
			m_Effect.transform.pImgTransform->GetWidth(), m_Effect.transform.pImgTransform->GetHeight(), SWP_NOZORDER | SWP_SHOWWINDOW | SWP_NOACTIVATE);
		SetLayeredAttributes(0, 0, LWA_ALPHA);
		m_Effect.transform.nFrame = 0;
		m_Effect.transform.nMaxFrame = nFrame;
		SetInternalTimer(TIMER_TRANSFORM, GIF_INTERVAL);		
	}
